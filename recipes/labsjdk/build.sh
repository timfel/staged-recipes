...
function source_build
{
  ...
  ./configure \
    ...
    --with-vendor-vm-buf-url=https://github.com/oracle/graal/issues \
    ${CONFIGURE_ARGS} \
    $_TOOLCHAIN_ARGS

  make JOBS=$CPU_COUNT $_TOOLCHAIN_ARGS || printerror
  make JOBS=$CPU_COUNT $_TOOLCHAIN_ARGS CONF_NAME=labsjdk graal-builder-image || printerror
}
...
if [[ "$target_platform" == linux* ]]; then
  ...
  jdk_install
  source_build
  cd build/labsjdk/images/graal-builder-jdk
fi
